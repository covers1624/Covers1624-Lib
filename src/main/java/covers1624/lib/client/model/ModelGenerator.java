    ...
    public void modelBakeEvent(ModelBakeEvent event) {
        ...
        for (Block block : blocks) {
            ...
            if (block instanceof IBlockTextureProvider) {
                ...
                for (int i = 0; i < tempStacks.size(); i++) {
                    ...
                    event.getModelRegistry().putObject(modelLocation, blockModel);
                    ...
                    event.getModelRegistry().putObject(inventoryModelLocation, inventoryModel);
        ...
        for (Item item : items) {
            if (item instanceof IItemTextureProvider) {
                ...
                for (int i = 0; i < tempStacks.size(); i++) {
                    ...
                    ItemLayerModel itemModel = new ItemLayerModel(builder.build());
                    IBakedModel model = itemModel.bake(ItemLayerModel.INSTANCE.getDefaultState(), DefaultVertexFormats.ITEM, textureGetter);
                    event.getModelRegistry().putObject(inventoryLocation, model);
        ...
        for (final BlockFluidClassic fluid : fluids) {
            ...
            for (int i = 0; i < 16; i++) {
                ...
                event.getModelRegistry().putObject(flowStateLocation, bakedModel);
            }
            ...
            event.getModelRegistry().putObject(inventoryModelLocation, bakedModel);
        }
        LogHelper.info("Finished Model Generation in %s Ms.", String.valueOf(System.currentTimeMillis() - start));

        //if (event.getModelRegistry() instanceof RegistrySimple) {
        //	for (ModelResourceLocation location : ((RegistrySimple<ModelResourceLocation, IBakedModel>) event.getModelRegistry()).getKeys()) {
        //		String locationString = location.toString();
        //		if (locationString.contains("diamond_sword")) {
        //			LogHelper.info(location);
        //			IBakedModel bakedModel = ForgeHooksClient.handleCameraTransforms(event.getModelRegistry().getObject(location), ItemCameraTransforms.TransformType.THIRD_PERSON);
